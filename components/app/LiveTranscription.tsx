...
const STT_PROXY_URL =
  typeof process.env.NEXT_PUBLIC_STT_WS_PROXY === "string"
    ? process.env.NEXT_PUBLIC_STT_WS_PROXY.replace(/\/$/, "").replace(/^http:/, "ws:")
    : "";
...
export function LiveTranscription() {
  const { isRecording, appendTranscript, setSessionId, setMicError } = useSession();
    ...
    (async () => {
      try {
        ...
        if (!tokenRes.ok) {
          ...
          else if (tokenRes.status === 404) msg = "Transcription isn't available in the standalone app. Use the app in your browser at localhost:3000 (run npm run dev), or run npm run desktop:dev with npm run dev in another terminal.";
          else {
            try {
        ...
        try {
          ...
          if (STT_PROXY_URL) {
            try {
              ws = await connectWebSocket(`${STT_PROXY_URL}/v1/listen?${query}`);
            } catch {
              ws = null!;
            }
          } else {
        ...
        const flushBuffer = () => {
          ...
          const text = phraseBufferRef.current.trim();
          phraseBufferRef.current = "";
          if (text && mounted) appendTranscript({ speaker: "user", text });
        };
        ...
        const tryReconnect = async () => {
          ...
          try {
            if (STT_PROXY_URL) {
              newWs = await connectWebSocket(`${STT_PROXY_URL}/v1/listen?${query}`);
            } else {
              const tokenRes = await fetch("/api/stt/token");
        ...
        const handleClose = (ev: CloseEvent) => {
          ...
          const remaining = phraseBufferRef.current.trim();
          phraseBufferRef.current = "";
          if (remaining && mounted) appendTranscript({ speaker: "user", text: remaining });
          if (keepaliveRef.current) {
            clearInterval(keepaliveRef.current);
            keepaliveRef.current = null;
          }
  ...
  }, [isRecording, appendTranscript, setSessionId, setMicError]);
